...
function tablemodel(tablename,dataObject,f){
   ...
   }else if(tablename==="aishangtcyyzl"){
    strd={
       "rq":dataObject.rq,
       "userid":dataObject.userid,
       "yycourceid":dataObject.yycourceid,
       "stu":dataObject.stu
        ...
        dbo.collection(table).find(wherestr).toArray(function(err, result) {  
           ...
           if(result.length===0)
            {   console.log("没有有此预约资料，即将添加")
                nomalinsert(database,table,cond)
                res.sendStatus(200)
  
            } 
            else{
                console.log("已经有此预约资料，即将删除")
                console.log(result[0]._id)
                nomaldelete(database,table,result[0]._id)
                res.sendStatus(200)
...
//插入操作
function nomalinsert(database,tablename,cond){
    MongoClient.connect(url, function(err, db) {
        var dbo = db.db(database);
        var f=cond.gz
        dbo.collection(tablename).insertOne(tablemodel(tablename,cond,f),(error,result)=>{ 
            console.log("我是插入程式")
            if(error)
            { 
                console.log('Error:'+ error);
...
////更新操作
function nomalupdate(database,tablename,wherestr){

}
///删除操作
function nomaldelete(database,tablename,wherestr)
{
    MongoClient.connect(url, function(err, db) {
        if (err) throw err;
        var dbo = db.db(database);
        let vvid=ObjectId(wherestr);
       
            dbo.collection(tablename).deleteOne({"_id":vvid},(err,result)=>{
...
/////
module.exports={dopost,doget,dodelete,checkexist,nomalinsert}